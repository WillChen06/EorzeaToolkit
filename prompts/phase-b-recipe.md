...
2. 首次載入中 → 顯示配方區塊的輕量 loading 狀態，不可先顯示「無法製作」
3. 查無 → 仍 render「配方」區塊，內容顯示「無法製作」
4. 查到 → 顯示配方陣列：
...
## 狀態處理
- `recipes.json` 為本地檔案，無網路請求
- 配方狀態需區分載入中與載入完成：
  - `nil` / 未完成載入 → 顯示輕量 loading
  - 空陣列 → 顯示「無法製作」
  - 非空陣列 → 顯示配方
- bundle 缺檔或 JSON 解碼失敗時，Debug 應留下診斷訊息，避免所有道具靜默退化為「無法製作」
